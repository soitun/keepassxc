...
namespace KeeShareSettings
{
    ...
    QString Sign::serialize(const Sign& sign)
    {
        ...
        BinaryStream stream(&rsaKeySerialized);
        stream.writeString(QString("ssh-rsa"));
        stream.writeString(QByteArray::fromRawData(reinterpret_cast<const char*>(rsaE.data()), rsaE.size()));
        stream.writeString(QByteArray::fromRawData(reinterpret_cast<const char*>(rsaN.data()), rsaN.size()));
        stream.flush();
